        ...
        if(isset($_GET['category']) || isset($_GET['input-min']) || isset($_GET['input-max'])){
            if(isset($idShop)) {
                for($i = 1; $i <= $totalPages; $i++) {
                    if($i != $currentPage){
                        echo "
                            <a href='./index.php?shopId=$idShop&current-page=".$i.$_SESSION['querystring']."' class='page-item'>$i</a>
                        ";
                    }
                    else{
                        echo"
            ...
            if(isset($idShop)) {
                for($i = 1; $i <= $totalPages; $i++) {
                    if($i != $currentPage){
                        echo "
                            <a href='./index.php?shopId=$idShop&current-page=".$i."' class='page-item'>$i</a>
                        ";
                    }
                    else{
                        echo"
